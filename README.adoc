...
. Run `:play objects` command, and click and run the Cypher statement to insert the dataset
...
----
include::src/main/java/objects/spring/data/neo4j/domain/Movie.java[tags=object]
...
----
include::src/main/java/objects/spring/data/neo4j/repositories/MovieRepository.java[tags=repository]
...
----
// JSON object for single object with cast
curl http://localhost:8080/objects?title=The%20Matrix

// list of JSON objects for object search results
curl http://localhost:8080/objects?title=*matrix*
